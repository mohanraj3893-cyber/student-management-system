// Cloudflare Pages Functions - Native Edge Serverless API Router
// Powered by Cloudflare D1 Database (env.DB)
...
export async function onRequest(context) {
  ...
  const db = env.DB;
  if (!db) {
    return jsonResponse({ message: 'D1 Database binding (DB) is missing in Cloudflare settings.' }, 500);
  }
  ...
  try {
    ...
    if (path === '/api/auth/register' && method === 'POST') {
      ...
      const isApproved = normalizedRole === 'admin' ? 1 : 0; // HOD auto-approved on setup
    ...
    // -------------------------------------------------------------
    // STUDENTS ROUTE: Get All / Filter Students
    // -------------------------------------------------------------
    ...
    // -------------------------------------------------------------
    // SUBJECTS ROUTE: Get / Create Subjects
    // -------------------------------------------------------------
    ...
    // -------------------------------------------------------------
    // ATTENDANCE ROUTE: Get Sessions & Mark Attendance
    // -------------------------------------------------------------
